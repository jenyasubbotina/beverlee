...
public class VerifyPinWorker extends Worker {
    ...
    public VerifyPinWorker(@NonNull Context context, @NonNull WorkerParameters workerParams) {
        ...
        this.pinCode = getInputData().getString(Constants.PINCODE);
    }
    ...
    public Result doWork() {
        ...
        try {
            final Response<Void> response = RetrofitClient.getInstance(getApplicationContext()).verifyPin(pinCode);
    ...
    private static final String TAG = VerifyPinWorker.class.toString();
}
